...
(file-subst "baro" "baric123456" "~/lisp/pg/myfile.txt" "~/lisp/pg/outputfile.txt")
...
(buf-pop my-buf);4 ;we have basically gotten to a b - the first letter - so we reset and dump the shit from the buffer here
...
; 1 - from the do macro - since "b a r b" didn't match "b a r o"
my-Buf

;;C-x 3 split
;;C-x 52 new window
;;
;; M-x slime - to open a slime window repl
;;ways to print to print
;;C-c C-c
;;C-c C-e
;;C-e E
;;C-e C-p ;will just evalute in separte window (we don't need to add it to the current program
;;C-x s ;; will save file
;;C-x C-c;; will close the file
;;;7.5 MACRO CHARACTERS
(car (read-from-string "'a")) ;see how the quote macro gets evaluated as soon as it is read.
;normally we don't see the (quote a) but when we call read explicitly
(cadr (read-from-string "'a"))

(car (read-from-string (funcall #'identity "#'min"))) ;returns a funciton - because of the #' means (function #'min)
(cadr (read-from-string (funcall #'identity "#'min"))) ;returns min

;;see how it returns a vector
(let ((*print-array* t))
  (vectorp (read-from-string (format nil "~S"
				     (vector 1 2))))) ;returns T since it (format with ~S) 
(read-from-string (format nil "~S"
			  (vector 1 2))) ;returns a vector displayed using the corresponding read-macros
(let ((*print-array* t))
  (vectorp (read-from-string (format nil "~S"
				     (vector 1 2)))))
